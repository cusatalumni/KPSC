...
const AdminPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    const runAudit = async () => {
        setLoading(true);
        try { const data = await adminOp('get-audit-report'); setAuditReport(data); setStatus("Audit Complete"); } 
        catch (e: any) { setStatus(e.message); setIsError(true); }
            ...
            <main className="bg-white dark:bg-slate-950 p-8 md:p-12 rounded-[3rem] shadow-2xl border dark:border-slate-800 min-h-[600px]">
                    ...
                    <div className="space-y-12">
                        ...
                        <div className="space-y-6">
                            <div className="flex items-center space-x-3 text-slate-800 dark:text-white">
                                ...
                                <h3 className="text-xl font-black uppercase tracking-tight">AI Protocols</h3>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                                {[
                                    ...
                                    { id: 'run-gap-filler', label: 'Auto Gap Filler', icon: SparklesIcon, color: 'bg-indigo-600' },
                                    ...
                                    >
                                        ...
                                        <div><p className="font-black text-sm dark:text-white">{tool.label}</p><p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Execute Protocol</p></div>
                                    </button>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="space-y-12">
                        <div className="bg-slate-950 p-10 rounded-[3rem] text-white shadow-2xl relative overflow-hidden group border border-white/5">
                             ...
                             <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
                                <div className="max-w-xl">
                                    <div className="flex items-center space-x-3 mb-4"><div className="p-2 bg-indigo-500 rounded-lg"><SparklesIcon className="h-6 w-6" /></div><span className="font-black tracking-widest text-xs uppercase text-indigo-400">Micro-Topic Intelligence</span></div>
                                    <h3 className="text-3xl font-black uppercase tracking-tighter">Syllabus Gap Audit</h3>
                                    <p className="text-slate-400 font-bold mt-3 text-sm leading-relaxed">Identify and fill syllabus gaps using Targeted AI. Limits usage automatically to stay within free tier.</p>
                                </div>
                                <div className="flex flex-col sm:flex-row gap-3">
                                    <button onClick={runAudit} className="bg-white/10 text-white border border-white/20 px-8 py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl hover:bg-white/20 transition-all flex items-center justify-center space-x-3"><ClipboardListIcon className="h-5 w-5" /><span>Show Report</span></button>
                                    <button onClick={() => handleAction(() => adminOp('run-batch-qa'))} className="bg-emerald-600 text-white px-8 py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-2xl hover:scale-105 active:scale-95 transition-all flex items-center justify-center space-x-3"><ShieldCheckIcon className="h-5 w-5" /><span>QA Audit</span></button>
                                </div>
                             </div>
                        </div>
                        ...
                        {auditReport.length > 0 && (
                            <div className="animate-fade-in space-y-8">
                                <h3 className="text-xl font-black uppercase tracking-tight px-2 flex items-center space-x-3"><ShieldCheckIcon className="h-6 w-6 text-indigo-500" /><span>Coverage Report</span></h3>
                                <div className="bg-white dark:bg-slate-900 border-2 border-slate-100 dark:border-slate-800 rounded-[2.5rem] overflow-hidden shadow-xl max-h-[600px] overflow-y-auto">
                                    <table className="w-full text-left">
                                        <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 font-black text-[10px] uppercase tracking-widest sticky top-0 z-20"><tr><th className="px-10 py-6">Micro-Topic</th><th className="px-6 py-6 text-center">Questions</th><th className="px-10 py-6 text-right">Status</th></tr></thead>
                                        <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                            {auditReport.sort((a,b)=>a.count-b.count).map(item => (
                                                <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                                    <td className="px-10 py-6"><div><p className="font-black text-sm dark:text-white leading-tight">{item.subject} • {item.topic}</p><p className="text-[10px] text-slate-400 font-bold mt-0.5">{item.title}</p></div></td>
                                                    <td className="px-6 py-6 text-center font-black text-xl text-slate-800 dark:text-slate-100">{item.count}</td>
                                                    <td className="px-10 py-6 text-right">
                                                        {item.count === 0 ? <span className="bg-red-50 text-red-600 border border-red-100 px-4 py-1.5 rounded-full text-[9px] font-black uppercase">Critical Gap</span> : item.count < 5 ? <span className="bg-amber-50 text-amber-600 border border-amber-100 px-4 py-1.5 rounded-full text-[9px] font-black uppercase">Low</span> : <span className="bg-emerald-50 text-emerald-600 border border-emerald-100 px-4 py-1.5 rounded-full text-[9px] font-black uppercase">Healthy</span>}
                                                    </td>
                                                </tr>
                                            ))}
                                        </tbody>
                        ...
                        <div className="bg-slate-50 dark:bg-slate-900/50 p-10 rounded-[3.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                             <div className="flex items-center space-x-4 mb-8"><CloudArrowUpIcon className="h-8 w-8 text-indigo-600" /><div><h3 className="text-2xl font-black uppercase tracking-tight">Bulk Import (CSV)</h3><p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Manual Upload</p></div></div>
                            <textarea value={csvContent} onChange={e => setCsvContent(e.target.value)} className="w-full h-48 p-6 rounded-3xl border-2 dark:bg-slate-800 font-mono text-[11px] mb-6 focus:border-indigo-500 outline-none" placeholder="id,topic,question,options,answer..." />
                ...
                {activeTab === 'access' && (
                    <div className="space-y-8">
                         <div className="flex items-center justify-between"><h3 className="text-2xl font-black uppercase tracking-tight">Active Subscriptions</h3><button onClick={() => refresh()} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-xl hover:bg-slate-200 transition-all"><ArrowPathIcon className="h-5 w-5" /></button></div>
                         <div className="bg-white dark:bg-slate-900 border-2 border-slate-50 dark:border-slate-800 rounded-[2.5rem] overflow-hidden shadow-xl">
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 dark:bg-slate-800 text-slate-500 font-black text-[10px] uppercase tracking-widest"><tr><th className="px-8 py-5">User ID</th><th className="px-8 py-5">Status</th><th className="px-8 py-5">Plan</th><th className="px-8 py-5">Expires</th><th className="px-8 py-5 text-right">Actions</th></tr></thead>
                                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                    {subscriptions.map(sub => (
                                        <tr key={sub.user_id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                            ...
                                            <td className="px-8 py-6 text-right"><button onClick={() => { if(confirm('Cancel subscription?')) handleAction(() => adminOp('delete-row', { sheet: 'Subscriptions', id: sub.user_id }))}} className="text-red-500 hover:scale-110 transition-transform"><TrashIcon className="h-5 w-5" /></button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div className="space-y-10">
                        <section className="bg-slate-50 dark:bg-slate-900 p-8 rounded-[2.5rem] border-2 border-slate-100 dark:border-slate-800">
                             ...
                             <form onSubmit={e => { e.preventDefault(); handleAction(() => adminOp('update-exam', { exam: newExam })); }} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <input value={newExam.category} onChange={e => setNewExam({...newExam, category: e.target.value})} placeholder="Category" className="p-4 rounded-2xl border-2 dark:bg-slate-800 font-bold" />
                ...
                {activeTab === 'syllabus' && (
                    <div className="space-y-12">
                         ...
                         <div className="bg-indigo-50 dark:bg-indigo-900/10 p-10 rounded-[3rem] border-2 border-dashed border-indigo-100 dark:border-indigo-800">
                            <textarea value={csvContent} onChange={e => setCsvContent(e.target.value)} className="w-full h-40 p-6 rounded-3xl border-2 dark:bg-slate-800 font-mono text-[11px] mb-6 focus:border-indigo-500 outline-none" placeholder="CSV: id,exam_id,title,questions,duration,subject,topic" />
                            <button onClick={() => handleBulkUpload('syllabus')} className="bg-indigo-600 text-white px-10 py-4 rounded-xl font-black uppercase tracking-widest text-[10px] shadow-lg">Import Syllabus</button>
                        </div>
                         {selectedExamId && (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {syllabusItems.map(item => (
                                        <div key={item.id} className="p-6 bg-white dark:bg-slate-900 border-2 border-slate-50 dark:border-slate-800 rounded-[2rem] flex justify-between items-center group">
                                            <div><p className="font-black text-base dark:text-white leading-tight">{item.title}</p><p className="text-[9px] text-indigo-50 font-black uppercase mt-1 px-2 py-0.5 bg-indigo-500 rounded-md inline-block">{item.subject} • {item.topic}</p></div>
