...
const AdminPage: React.FC<{ onBack: () => void }> = ({ onBack }) => {
    ...
    const refresh = useCallback(async (isSilent = false) => {
        try {
            ...
            const [settings, conn] = await Promise.all([
                getSettings(true),
                testConnection(token)
            ]);

            // Test connection returns the actual status from the backend check
            if (conn && conn.status) setDbStatus(conn.status);
            
            // Fetch exams separately to handle fallback logic clearly
    ...
    return (
        <div className="max-w-7xl mx-auto space-y-10 pb-24 px-4 animate-fade-in">
            ...
            <div className="flex flex-col md:flex-row justify-between items-center bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 shadow-xl gap-6">
                ...
                <div className="flex flex-wrap items-center justify-center gap-8">
                    <div className="flex items-center space-x-6">
                        ...
                        <div className="flex items-center space-x-2">
                            <div className={`w-3 h-3 rounded-full ${dbStatus.supabase ? 'bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.5)]' : 'bg-slate-400 shadow-[0_0_10px_rgba(0,0,0,0.1)]'}`}></div>
                    ...
                    <div className="flex items-center space-x-4">
                        <div className="bg-slate-50 dark:bg-slate-800 px-4 py-2 rounded-xl border border-slate-100 dark:border-slate-700">
                             <p className="text-[9px] font-black text-slate-400 uppercase tracking-tighter">Active Exams Source</p>
                             <p className={`text-[10px] font-black uppercase ${dataSource === 'Database' ? 'text-indigo-600' : 'text-amber-500'}`}>{dataSource}</p>
                        </div>
                        <button onClick={() => refresh()} className="p-3 bg-indigo-600 rounded-xl text-white hover:bg-indigo-700 transition-all shadow-lg active:scale-90">
            ...
            <header className="bg-slate-950 p-12 rounded-[4rem] text-white shadow-2xl border-b-[10px] border-indigo-600 flex justify-between items-center relative overflow-hidden ring-1 ring-white/10">
                ...
                <div className="flex items-center space-x-8 relative z-10">
                    ...
                    <div className="space-y-1">
                        ...
                        <p className="text-indigo-400 text-[11px] font-black tracking-[0.4em] uppercase opacity-80">Operational Headquarters</p>
                    </div>
                </div>
            </header>
            ...
            <div className="flex flex-wrap gap-4 overflow-x-auto pb-4 scrollbar-hide">
                ...
                {tabBtn('exams', 'Active Exams', AcademicCapIcon)}
                {tabBtn('syllabus', 'Course Map', ClipboardListIcon)}
                {tabBtn('questions', 'Q-Bank', PlusIcon)}
                {tabBtn('bookstore', 'Store', BookOpenIcon)}
                {tabBtn('subscriptions', 'Premium Control', StarIcon)}
            </div>
            ...
            <main className="bg-white dark:bg-slate-900 p-12 rounded-[4rem] shadow-2xl border border-slate-100 dark:border-slate-800 min-h-[700px] ring-1 ring-slate-100 dark:ring-slate-800">
                {activeTab === 'automation' && (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10">
                        {[
                            { icon: RssIcon, color: 'bg-indigo-600', title: 'Global Refresh', desc: 'Sync Notifications, GK & News from PSC Sources', action: () => triggerDailyScraper },
                            { icon: BookOpenIcon, color: 'bg-emerald-600', title: 'Amazon Sync', desc: 'Refetch latest PSC rank files from marketplace', action: () => triggerBookScraper },
                            { icon: ArrowPathIcon, color: 'bg-rose-600', title: 'Clear AI Cache', desc: 'Purge temporary AI generated study materials', action: () => clearStudyCache }
                        ].map((card, i) => (
                ...
                {activeTab === 'exams' && (
                    <div className="space-y-10">
                        <div className="flex justify-between items-center border-b border-slate-100 dark:border-slate-800 pb-8">
                             <h3 className="text-3xl font-black tracking-tighter">Live Exams Registry</h3>
                             <div className="bg-indigo-50 dark:bg-indigo-900/30 px-6 py-2.5 rounded-full border border-indigo-100 dark:border-indigo-800">
                                 <span className="text-[11px] font-black text-indigo-600 dark:text-indigo-400 uppercase tracking-widest">{exams.length} Managed Entities</span>
                             </div>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {exams.map(ex => (
                                <div key={ex.id} className="flex items-center justify-between p-7 border-2 rounded-[2.5rem] bg-slate-50 dark:bg-slate-950 group hover:border-indigo-500 hover:bg-white dark:hover:bg-slate-900 transition-all duration-500 shadow-sm hover:shadow-xl">
                                    ...
                                    <button onClick={async () => { if(confirm("Permanently remove this exam from the registry?")) handleAction(async () => deleteBook(ex.id, await getToken())) }} className="text-red-500 hover:bg-red-50 p-3.5 rounded-2xl transition-all opacity-0 group-hover:opacity-100 active:scale-75"><TrashIcon className="h-6 w-6" /></button>
                                </div>
                            ))}
                        </div>
                        {exams.length === 0 && (
                    ...
                    <div className="space-y-16">
                        <section className="bg-slate-950 p-12 rounded-[3.5rem] border border-white/5 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.5)] relative overflow-hidden">
                            ...
                            <form onSubmit={handleSingleQuestion} className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                                ...
                                <button type="submit" disabled={loading} className="md:col-span-2 bg-indigo-600 hover:bg-indigo-500 text-white py-7 rounded-[2rem] font-black shadow-2xl tracking-[0.4em] text-xs uppercase transition-all active:scale-95 flex items-center justify-center space-x-4">
                        ...
                        <section className="pt-16 border-t-4 border-slate-50 dark:border-slate-800">
                            ...
                            <button onClick={async () => handleAction(async () => syncCsvData('QuestionBank', bulkData, await getToken(), true))} className="w-full mt-8 bg-emerald-600 text-white py-6 rounded-[2rem] font-black text-xs shadow-[0_20px_50px_rgba(16,185,129,0.3)] tracking-[0.3em] hover:bg-emerald-500 transition-all uppercase active:scale-95">Initiate Bulk Append</button>
                        </section>
                    </div>
                )}
                ...
                {activeTab === 'bookstore' && (
                    <div className="space-y-10">
                        <div className="flex justify-between items-center border-b border-slate-50 dark:border-slate-800 pb-8">
                            ...
                            <button onClick={async () => handleAction(async () => triggerBookScraper(await getToken()))} className="bg-emerald-600 text-white px-8 py-4 rounded-2xl font-black shadow-xl uppercase tracking-widest hover:scale-105 active:scale-90 transition-all text-[11px]">Sync Amazon Marketplace</button>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                            {books.map(b => (
                                <div key={b.id} className="flex items-center p-7 border-2 rounded-[3rem] bg-white dark:bg-slate-950 group hover:border-emerald-500 hover:shadow-2xl transition-all duration-500 relative">
                                    ...
                                    <button onClick={async () => { if(confirm("Are you sure you want to remove this book from the storefront?")) handleAction(async () => deleteBook(b.id, await getToken())) }} className="text-red-500 p-3 rounded-2xl hover:bg-red-50 transition-all opacity-0 group-hover:opacity-100"><TrashIcon className="h-6 w-6" /></button>
                                </div>
                            ))}
                        </div>
                        {books.length === 0 && (
                            <div className="text-center py-32 opacity-20 italic font-black text-2xl uppercase tracking-widest">Storefront Empty</div>
                        )}
                    </div>
                )}
                    ...
                    <div className="max-w-xl mx-auto py-24 text-center space-y-12">
                        <div className={`p-16 rounded-[5rem] border-4 transition-all duration-700 shadow-[0_50px_100px_-20px_rgba(0,0,0,0.2)] ${isSubscriptionActive ? 'bg-indigo-50 border-indigo-200' : 'bg-slate-50 border-slate-200 shadow-none'}`}>
                            ...
                            <button 
                                ...
                                className={`w-full py-7 rounded-[2.5rem] font-black text-xs tracking-[0.4em] uppercase transition-all duration-500 shadow-2xl active:scale-95 ${isSubscriptionActive ? 'bg-indigo-600 text-white' : 'bg-slate-950 text-white'}`}
                            >
                                {isSubscriptionActive ? 'Deactivate Paywall' : 'Enable Paywall System'}
                            </button>
                        </div>
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-[0.5em] opacity-40">Security Clearance Level: Admin</div>
                    </div>
                )}
                    ...
                    <div className="space-y-12">
                        ...
                        <div className="space-y-8">
                             <div className="flex items-center justify-between ml-2">
                                <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.4em]">Structure Import</p>
                                <a href="#" className="text-[9px] font-black text-indigo-500 underline uppercase tracking-tighter">View Format Guide</a>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <button onClick={async () => handleAction(async () => syncCsvData('Syllabus', syllabusBulkData, await getToken(), true))} className="bg-indigo-600 text-white py-6 rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] shadow-[0_20px_40px_rgba(79,70,229,0.3)] hover:bg-indigo-500 transition-all active:scale-95">Append to Map</button>
                                <button onClick={async () => { if(confirm("This will wipe all existing mappings for ALL exams. Are you absolutely sure?")) handleAction(async () => syncCsvData('Syllabus', syllabusBulkData, await getToken(), false)) }} className="bg-slate-200 dark:bg-slate-800 text-slate-700 dark:text-slate-300 py-6 rounded-[2rem] font-black text-xs uppercase tracking-[0.3em] hover:bg-slate-300 transition-all active:scale-95">Replace Global Map</button>
                            </div>
                        </div>
                        <div className="mt-16 space-y-6">
                            <h4 className="text-[11px] font-black uppercase text-slate-400 tracking-[0.5em] mb-8 ml-2">Current Map Nodes</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {syllabusItems.map(s => (
                                    <div key={s.id} className="flex items-center justify-between p-6 bg-white dark:bg-slate-950 border-2 border-slate-50 dark:border-slate-800 rounded-[2rem] shadow-sm group hover:border-indigo-500 transition-all duration-300">
                                        ...
                                        <button onClick={async () => { if(confirm("Remove this mapping?")) handleAction(async () => deleteBook(s.id, await getToken())) }} className="text-red-500 p-3 opacity-0 group-hover:opacity-100 transition-all hover:bg-red-50 rounded-2xl active:scale-75"><TrashIcon className="h-5 w-5" /></button>
                                    </div>
                                ))}
                            </div>
                        </div>
