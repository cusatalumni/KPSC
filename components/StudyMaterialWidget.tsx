...
interface StudyMaterialWidgetProps {
    onStartStudy: (topic: string) => void;
}

const StudyMaterialWidget: React.FC<StudyMaterialWidgetProps> = ({ onStartStudy }) => {
    return (
        <div className="bg-white p-6 rounded-xl shadow-lg border border-slate-100">
            ...
            <div className="space-y-2">
                {STUDY_MATERIALS_DATA.map((material) => (
                    <button
                        key={material.id}
                        onClick={() => onStartStudy(material.title)}
                        className="w-full flex items-center space-x-3 p-3 rounded-lg hover:bg-slate-100 transition-colors duration-200 text-left"
                    >
