...
const TestPage: React.FC<TestPageProps> = ({ activeTest, subscriptionStatus, onTestComplete, onBack, onNavigateToUpgrade }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center h-screen bg-slate-50 dark:bg-slate-900">
        ...
        <div className="mt-8 text-center">
            <p className="text-2xl font-black text-slate-800 dark:text-white tracking-tight">{t('test.loading.ml')}</p>
  ...
  if (error || questions.length === 0) {
    return (
        <div className="flex flex-col items-center justify-center h-screen text-center p-4 bg-slate-50 dark:bg-slate-950">
            <div className="bg-white dark:bg-slate-900 p-10 rounded-[2.5rem] shadow-xl border border-slate-200 dark:border-slate-800">
  ...
  return (
    <div ref={testPageRef} className="bg-slate-50 dark:bg-slate-950 min-h-screen flex flex-col items-center py-8 px-4 sm:px-6">
      <div className="w-full max-w-4xl animate-fade-in-up">
        {!isSignedIn && (
          <div className="bg-amber-100 dark:bg-amber-900/20 border-l-4 border-amber-500 text-amber-800 dark:text-amber-400 p-4 mb-6 rounded-r-2xl text-sm flex items-center space-x-4 shadow-sm">
              <UserCircleIcon className="h-6 w-6 flex-shrink-0 text-amber-600" />
              <p className="font-bold">{t('test.guestMode')}</p>
          </div>
        )}

        <div className="bg-white dark:bg-slate-900 p-8 sm:p-12 rounded-[2.5rem] shadow-2xl border border-slate-100 dark:border-slate-800 relative overflow-hidden glass-card transition-colors">
            {/* Header */}
            <header className="mb-10 border-b border-slate-100 dark:border-slate-800 pb-8">
              <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                  <div className="flex items-center space-x-5">
                    ...
                    <div>
                      <h1 className="text-2xl font-black text-slate-800 dark:text-white line-clamp-1 tracking-tight">{activeTest.title}</h1>
                      <div className="flex items-center text-xs text-slate-400 font-bold uppercase tracking-wider space-x-3 mt-1">
                        ...
                        <span>•</span>
                        <span className="text-indigo-600 dark:text-indigo-400">{currentQuestion.subject || 'General'}</span>
                        {currentQuestion.difficulty && (
                          <>
                            <span>•</span>
                            <span className="text-emerald-600 dark:text-emerald-400">{currentQuestion.difficulty}</span>
                          </>
                        )}
                      </div>
                    </div>
                  ...
                  <div className="flex items-center space-x-4">
                      <div className={`flex items-center space-x-2 font-mono font-black px-6 py-3 rounded-2xl border-2 transition-all ${timeLeft < 300 ? 'text-red-600 bg-red-50 dark:bg-red-950/20 border-red-200 animate-pulse' : 'text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800 border-slate-100 dark:border-slate-700'}`}>
                          <ClockIcon className="h-6 w-6"/>
                          <span className="text-2xl">{formatTime(timeLeft)}</span>
                      </div>
                      <button onClick={toggleFullscreen} className="p-3 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-2xl transition-all">
              ...
              <div className="mt-8">
                  ...
                  <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-3 p-1 shadow-inner">
                ...
                <label 
                  ...
                  className={`flex items-center p-6 rounded-2xl border-2 cursor-pointer transition-all duration-300 group ${
                    answers[currentIndex] === index 
                      ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-600 shadow-xl shadow-indigo-100 dark:shadow-none scale-[1.03] ring-4 ring-indigo-50 dark:ring-indigo-900/30' 
                      : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700 hover:border-indigo-200 dark:hover:border-indigo-800 hover:bg-slate-50 dark:hover:bg-slate-800/80'
                  }`}
                >
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center mr-5 border-2 font-black text-lg transition-all ${
                       ...
                       : 'bg-slate-50 dark:bg-slate-900 text-slate-400 border-slate-200 dark:border-slate-700 group-hover:border-indigo-300 group-hover:bg-indigo-50'
                    }`}>
                    ...
                    <input 
                        ...
                        className="hidden"
                    />
                    <span className={`text-xl font-bold ${answers[currentIndex] === index ? 'text-indigo-900 dark:text-indigo-100' : 'text-slate-700 dark:text-slate-300'}`}>
                      {option}
                    </span>
                </label>
            ))}
            ...
            {/* Controls */}
            <footer className="mt-16 pt-10 border-t border-slate-100 dark:border-slate-800 flex flex-wrap justify-between items-center gap-6">
                <button 
                    onClick={() => setCurrentIndex(prev => prev - 1)} 
                    disabled={currentIndex === 0}
                    className="flex-1 sm:flex-none order-2 sm:order-1 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 font-black py-5 px-10 rounded-2xl hover:bg-slate-200 dark:hover:bg-slate-700 disabled:opacity-30 transition-all active:scale-95"
                >
                    {t('test.previous')}
                </button>
      ...
      >
        <p className="font-medium text-slate-600 dark:text-slate-400">{t('upgradeModal.body')}</p>
      </Modal>
    </div>
  );
};
