...
export default async function handler(req: any, res: any) {
    ...
    // Results can be saved without admin token for performance
    ...
    try {
        switch (action) {
            ...
            case 'add-question':
                const qRow = [`q_${Date.now()}`, question.topic, question.question, JSON.stringify(question.options), question.correctAnswerIndex, question.subject || 'GK', 'PSC Level'];
            ...
            case 'csv-update':
                const rows = data.split('\n').filter((l:string) => l.trim() !== '').map((line: string) => {
                    ...
                    if (sheet === 'QuestionBank') {
                        // Topic, Question, Opts(A|B|C|D), CorrectIdx, Subject, Diff
            ...
            case 'fix-affiliates':
                await runBookScraper(); // Re-runs book scraper which has affiliate logic
                return res.status(200).json({ message: 'Links verified.' });
