...
export default async function handler(req: any, res: any) {
    // Handle Vercel Cron (GET request)
    if (req.method === 'GET') {
        const authHeader = req.headers.authorization;
        if (authHeader !== `Bearer ${process.env.CRON_SECRET}`) {
            return res.status(401).json({ message: 'Unauthorized Cron attempt.' });
        }
        
        console.log("Starting Scheduled Auto-Update Engine...");
        try {
            const result = await runDailyUpdateScrapers();
            return res.status(200).json({ message: 'Cron Job Success', result });
        } catch (e: any) {
            console.error("Scheduled Scraper Error:", e.message);
            return res.status(500).json({ message: 'Cron Job Failed', error: e.message });
        }
    }
