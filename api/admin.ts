    ...
    try {
        switch (action) {
            case 'sync-all':
                ...
                await upsertSupabaseData('questionbank', qbs.filter(r => r[0]).map(r => ({ id: r[0], topic: r[1], question: r[2], options: smartParseOptions(r[3]), correct_answer_index: r[4], subject: r[5], difficulty: r[6] })));
                ...
                await upsertSupabaseData('liveupdates', ups.filter(r => r[0]).map(r => ({ id: createNumericHash(r[1] || r[0]), title: r[0], url: r[1], section: r[2], published_date: r[3] })));
                await upsertSupabaseData('studymaterialscache', smc.filter(r => r[0]).map(r => ({ topic: r[0], content: r[1], last_updated: r[2] })), 'topic');

                return res.status(200).json({ message: 'Global Cloud Architecture Synced Successfully.' });
