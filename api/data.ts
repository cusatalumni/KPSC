...
const smartParseOptions = (raw: any): string[] => {
    ...
    if (clean.startsWith('[') && clean.endsWith(']')) {
        try { return JSON.parse(clean.replace(/'/g, '"')); } catch (e) {
...
export default async function handler(req: any, res: any) {
    ...
    const sheetTab = sheetTabMap[tType] || type.charAt(0).toUpperCase() + type.slice(1);
    ...
    try {
        ...
        // Fallback to Google Sheets with detailed mapping
        switch (tType) {
            case 'settings':
            ...
            case 'exams':
                ...
                return res.status(200).json(eRows.map(r => ({ id: r[0], title_ml: r[1], title_en: r[2], description_ml: r[3], category: r[5], level: r[6], icon_type: r[7] })));
            case 'books':
            ...
            case 'syllabus':
                ...
                const filtered = examId ? sylRows.filter(r => r[1] === examId) : sylRows;
                return res.status(200).json(filtered.map(r => ({ id: r[0], exam_id: r[1], title: r[2], questions: parseInt(r[3]), duration: parseInt(r[4]), subject: r[5], topic: r[6] })));
            case 'questions':
            ...
            default:
                ...
                const genericRows = await readSheetData(`${sheetTab}!A2:Z`);
                return res.status(200).json(genericRows);
        }
    } catch (error: any) {
        console.error(`Data API Error (${tType}):`, error.message);
